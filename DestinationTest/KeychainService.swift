...
//  DestinationTest
//
//  Created by Jair Moreno Gaspar on 4/28/19.
...
let kSecAttrServiceValue = NSString(format: kSecAttrService) //Identificador del item (key)
...
public class KeychainService: NSObject {
    ...
    class func savePassword(service: String, account:String, data: String) {
        if let dataFromString = data.data(using: String.Encoding.utf8, allowLossyConversion: false) {
            ...
            // Instantiate a new default keychain query
            let keychainQuery: NSMutableDictionary = NSMutableDictionary(objects: [kSecClassGenericPasswordValue, service, account, dataFromString, generalGroup], forKeys: [kSecClassValue, kSecAttrServiceValue, kSecAttrAccountValue, kSecValueDataValue, kSecGroup])
